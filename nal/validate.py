    ...
    with open(data_location + on_file) as csv_file:
        ...
        for i, line in enumerate(rotten_tomatoes):
            if not i:
                ...
                movies[lookup[primary_column]] = [
                    set(lookup[on_column].strip().split(", ")),
                    int(lookup[ratings_column] if lookup[ratings_column] else 0),
                ]
    for movie in edgelist:
        for connection in edgelist[movie]:
            assert movies[movie][0].intersection(movies[connection][0])
            assert (
                abs(movies[movie][1] - movies[connection][1])
                == edgelist[movie][connection]["weight"]
            )
